  ...
  document.querySelectorAll(selector).forEach((link) => {
    function handler(e) {
      ...
      if (target) {
        ...
        setTimeout(() => target.classList.remove("section-animate"), 1e3);
      }
    }
...
const yearsScroll = () => {
  ...
  for (let i = 1; i < layers.length; i++) {
    ...
    tl.to(
      ...
      { yPercent: -40, autoAlpha: 0, stagger: 0, duration: 0.2 },
      ">"
    );
    tl.set(next, { autoAlpha: 1 }, "<");
    tl.set(
      next.querySelectorAll(".digit"),
      { yPercent: fromTop ? -100 : 100, autoAlpha: 0 },
      "<"
    );
    tl.to(
      next.querySelectorAll(".digit"),
  ...
  ScrollTrigger.create({
    ...
    onLeaveBack: () => {
      ...
      if (!lockOpen) {
        ...
        gsap.to(lock, { scale: 1, duration: 0, overwrite: true });
      }
      fadeInDone = false;
...
const stopLockAnimation = () => {
  ...
  gsap.killTweensOf(lock);
  gsap.to(lock, { x: 0, rotation: 0, duration: 0.2, ease: "power2.out" });
};
const animateLock = () => {
